...
const RightSideBar = () => {
  const [showSponser, setshowSponser] = useState(false);
  const sponsors = [
    {
      name: "Netflix",
  ...
  const displaySponsers = showSponser ? sponsors : sponsors.slice(0, 2);
  return (
    <motion.aside
        ...
        <CardContent>
          <ul className="space-y-4">
            {displaySponsers.map((sponser, index) => (
              <motion.li
                ...
                className="flex flex-col items-center space-x-4 rounded-lg p-2 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors duration-200">
                <img
                  src={sponser.image}
                  alt={sponser.name}
                  className="object-contain h-32 w-50 rounded-md"
                />
                <div className="flex-1">
                  <h3 className="font-semibold  text-lg">{sponser.name}</h3>
                  ...
                  <a
                    ...
                    visit website <ExternalLink className="h-4 w-4 ml-4" />{" "}
                  </a>
                </div>
              </motion.li>
            ))}
          ...
          {sponsors.length > 2 && (
            <Button
              ...
              onClick={() => setshowSponser(!showSponser)}>
              {showSponser ? "Show Less" : "Show More"}
            </Button>
          )}
        </CardContent>
      </Card>
